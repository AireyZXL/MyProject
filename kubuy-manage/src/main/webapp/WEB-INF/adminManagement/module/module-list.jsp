	 ...
	 $('.table-sort').dataTable(
            {
               ...
               "bProcessing": true, //开启读取服务器数据时显示正在加载中……特别是大数据量的时候，开启此功能比较好
                ...
                "columns": [ //这个属性下的设置会应用到所有列，按顺序没有是空
                   {"data":"moduleId","render": function(data, type, full) { // 返回自定义内容
                	  return '<input type="checkbox" name="" value="'+data+'">';
           			}},
                    {"data": 'moduleId'}, //mData 表示发请求时候本列的列名，返回的数据中相同下标名字的数据会填充到这一列
                    {"data":"moduleName","render": function(data, type, full) { // 返回自定义内容
   						if(data==null){
   							return "";
   						}
                    	return '<u style="cursor:pointer" class="text-primary" onclick="member_show(\'张三\',\'member-show.html\',\'10001\',\'360\',\'400\')">'+data+'</u>'
                	}},
                	{"data":"ctype","render": function(data, type, full) { // 返回自定义内容
                         if(data===1)
                         return "<span class='label label-success'>一级菜单</span>";
                         else if(data===2)
                         return "<span class='label label-defaunt radius'>二级菜单</span>";
                         else if(data===3)
                             return "<span class='label label-defaunt radius'>我的按钮</span>";
                	}},
                    {"data": 'remark'},
                    ...
                    {"data":"moduleId","sClass": "text-center","render": function(data, type, full) { // 返回自定义内容
                       var id=data;
